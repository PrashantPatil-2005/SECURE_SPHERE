...
class SecuriSphereEvaluator:
    ...
    def run_named_scenarios(self):
        """
        Run the three formally-named attack scenarios (A, B, C) and calculate
        metrics using the accurate MTTD endpoint rather than the approximation.
        """
        self.print_header("SecuriSphere Named-Scenario Evaluation (A / B / C)")
        ...
        try:
            ...
            print(f"Could not import scenario modules: {exc}")
            return

        # Import kill chain tagger and baseline MTTD measurement
        _tag_kill_chain = None
        _measure_raw_log_mttd = None
        try:
            sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'engine'))
            from kill_chain.reconstructor import tag_kill_chain as _tag_kill_chain
        except Exception as exc:
            print(f"Warning: Could not import tag_kill_chain: {exc}")
        try:
            from baseline_mttd import measure_raw_log_mttd as _measure_raw_log_mttd
        except Exception as exc:
            print(f"Warning: Could not import baseline_mttd: {exc}")

        named = [
            ("Scenario A — Brute Force → Credential Compromise → Exfiltration", run_a),
            ("Scenario B — Recon → SQL Injection → Privilege Escalation",        run_b),
            ("Scenario C — Multi-Hop Lateral Movement",                           run_c),
        ]

        mttd_rows = []
        for label, fn in named:
            self.clear_system()
            self.print_section(label)
            before = self.get_metrics_snapshot()
            start  = time.time()

            try:
                fn(delay_multiplier=0.3)   # fast mode for evaluation
            except Exception as exc:
            ...
            result = self.calculate_scenario_metrics(
                label, label, before, after, {}, 1, duration
            )
            self.evaluation_results['scenarios'].append(result)
            print(f"Result: {result['incidents']['count']} incidents | MTTD≈{result['metrics']['mean_time_to_detect_seconds']}s")

            # Tag new incidents with scenario label and measure MTTD
            incident_ids_before = {i['incident_id'] for i in before.get('incidents', [])}
            new_incidents = [
                i for i in after.get('incidents', [])
                if i['incident_id'] not in incident_ids_before
            ]

            if _tag_kill_chain:
                for inc in new_incidents:
                    try:
                        _tag_kill_chain(inc['incident_id'], label)
                        print(f"  Tagged kill chain {inc['incident_id']} as '{label}'")
                    except Exception as e:
                        print(f"  Warning: Could not tag kill chain: {e}")

            ss_mttd_values = [
                inc.get('mttd_seconds')
                for inc in new_incidents
                if inc.get('mttd_seconds') is not None
            ]
            ss_mttd = min(ss_mttd_values) if ss_mttd_values else None

            baseline_mttd = None
            if _measure_raw_log_mttd:
                try:
                    baseline_mttd = _measure_raw_log_mttd(label)
                except Exception as e:
                    print(f"  Warning: Baseline MTTD measurement failed: {e}")

            if not hasattr(self, '_mttd_results'):
                self._mttd_results = []
            self._mttd_results.append({
                "scenario": label,
                "incidents_detected": len(new_incidents),
                "ss_mttd_seconds": ss_mttd,
                "baseline_mttd_seconds": baseline_mttd,
                "reduction_pct": (
                    round((1 - ss_mttd / baseline_mttd) * 100, 1)
                    if ss_mttd and baseline_mttd and baseline_mttd > 0
                    else None
                ),
            })

            # Pull accurate MTTD from backend if PostgreSQL is populated
            try:
                mttd_resp = requests.get(f"{self.backend}/api/mttd/report", timeout=5)
                if mttd_resp.status_code == 200:
                    mttd_rows = mttd_resp.json().get("data", [])
            except Exception:
                pass
        ...
        if mttd_rows:
            self.print_section("Accurate MTTD Report (from PostgreSQL / Redis)")
            headers = ["Incident Type", "Count", "Avg MTTD (s)", "Min (s)", "Max (s)"]
            table   = [
                [r.get("incident_type"), r.get("incident_count"),
                 r.get("avg_mttd_seconds"), r.get("min_mttd_seconds"), r.get("max_mttd_seconds")]
                for r in mttd_rows
            ]
        ...
        # Print MTTD experiment results table
        if hasattr(self, '_mttd_results') and self._mttd_results:
            print("\n" + "="*72)
            print("  MTTD EXPERIMENT RESULTS")
            print("="*72)
            print(f"  {'Scenario':<42} {'Baseline':>10} {'SecuriSphere':>14} {'Reduction':>10}")
            print(f"  {'-'*42} {'-'*10} {'-'*14} {'-'*10}")
            for row in self._mttd_results:
                baseline_str = f"{row['baseline_mttd_seconds']:.1f}s" if row['baseline_mttd_seconds'] and row['baseline_mttd_seconds'] > 0 else "N/A"
                ss_str = f"{row['ss_mttd_seconds']:.1f}s" if row['ss_mttd_seconds'] is not None else "N/A"
                red_str = f"{row['reduction_pct']}%" if row['reduction_pct'] is not None else "N/A"
                print(f"  {row['scenario']:<42} {baseline_str:>10} {ss_str:>14} {red_str:>10}")
            print("="*72)
            print("  Run 'make mttd-markdown' to export this table for your research paper.")
            print()
