...
class NetworkMonitor:
    ...
    def start(self):
        ...
        # Exit non-zero on failure so Docker's restart policy recreates us; scapy's
        # sniff() returns cleanly when its socket dies (e.g. shared netns goes away
        # during api-server restart), which would otherwise exit 0 and stay dead.
        try:
            ...
            print(f"{Colors.RED}[!] sniff() crashed: {e}{Colors.RESET}")
            raise SystemExit(3)

        # sniff() returned without raising — socket died silently. Exit non-zero
        # so restart: unless-stopped brings us back with a fresh socket.
        print(f"{Colors.RED}[!] sniff() exited (socket closed). Exiting for restart.{Colors.RESET}")
        raise SystemExit(1)
